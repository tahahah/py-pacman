...
// Add this function to the Map class
Map.prototype.getNumericTile = function(x, y) {
    const tile = this.getTile(x, y);
    
    // Find character positions if not already found
    if (!this._characterPositions) {
        this._characterPositions = this.findGhostHouse();
    }
    
    // Check for character positions
    if (this._characterPositions) {
        // Check for Pacman
        if (x === this._characterPositions.pacman.x && y === this._characterPositions.pacman.y) {
            return 40; // Pacman position
        }
        // Check for ghosts
        for (let i = 0; i < this._characterPositions.ghosts.length; i++) {
            const ghost = this._characterPositions.ghosts[i];
            if (x === ghost.x && y === ghost.y) {
                return 33 + i; // Ghost positions (33-36)
            }
        }
    }
    
    // Check for teleport tiles (at the edges of the maze)
    if (x === -1 || x === this.numCols || (tile === '_' && (x === 0 || x === this.numCols-1))) {
        return 50; // teleport tile
    }
    
    // First determine what type of tile it is based on surrounding tiles
    const isWall = tile === '|';
    ...
    if (tile === '.') {
        // Check if it's a pellet position
        return 14; // pellet.gif
    }
    else if (tile === '-') {
        return 13; // ghost-door.gif
    }
    else if (tile === '_' || tile === ' ') {
        return 10; // blank.gif
    }
        ...
        if (up && down && !left && !right) return 26; // wall-straight-vert.gif
        if (!up && !down && left && right) return 25; // wall-straight-horiz.gif
        if (!up && !left && right && down) return 18; // wall-corner-ul.gif
        if (!up && !right && left && down) return 19; // wall-corner-ur.gif
        if (!down && !left && up && right) return 16; // wall-corner-ll.gif
        if (!down && !right && up && left) return 17; // wall-corner-lr.gif
        if (up && down && right && !left) return 28; // wall-t-left.gif
        if (up && down && !right && left) return 29; // wall-t-right.gif
        if (up && !down && right && left) return 27; // wall-t-bottom.gif
        if (!up && down && right && left) return 30; // wall-t-top.gif
        if (up && down && right && left) return 31; // wall-x.gif
        if (!up && down && !right && !left) return 23; // wall-end-t.gif
        if (up && !down && !right && !left) return 20; // wall-end-b.gif
        if (!up && !down && right && !left) return 21; // wall-end-l.gif
        if (!up && !down && !right && left) return 22; // wall-end-r.gif
        return 24; // wall-nub.gif for any other wall configuration
    }
    
    return 10; // default to blank.gif instead of 50
};

Map.prototype.findGhostHouse = function() {
    // Find the ghost door position first
    let doorPos = null;
    ...
    // Find a valid position for Pacman
    let pacmanPos = null;
    
    // First try to find the ideal pattern (14, 10, 10, 14)
    for (let y = 0; y < this.numRows; y++) {
        for (let x = 0; x < this.numCols - 3; x++) {
            // Check if we have a sequence of (pellet, blank, blank, pellet)
            if (this.getTile(x, y) === '.' &&           // pellet (14)
                (this.getTile(x+1, y) === '_' || this.getTile(x+1, y) === ' ') && // blank (10)
                (this.getTile(x+2, y) === '_' || this.getTile(x+2, y) === ' ') && // blank (10)
                this.getTile(x+3, y) === '.') {         // pellet (14)
                // Place Pacman on the first blank tile
                pacmanPos = {x: x+1, y};
                break;
            }
        }
    ...
    // If ideal pattern not found, fall back to finding any blank tile with adjacent pellet
    if (!pacmanPos) {
        for (let y = 0; y < this.numRows; y++) {
            for (let x = 0; x < this.numCols; x++) {
                const tile = this.getTile(x, y);
                if (tile === '_' || tile === ' ') {  // Check if it's a blank tile
                    // Check if at least one adjacent tile is a pellet
                    const hasAdjacentPellet = 
                        this.getTile(x-1, y) === '.' ||
                        this.getTile(x+1, y) === '.' ||
                        this.getTile(x, y-1) === '.' ||
                        this.getTile(x, y+1) === '.';
                    
                    if (hasAdjacentPellet) {
                        pacmanPos = {x, y};
                        break;
                    }
                }
    ...
    return {
        door: doorPos,
        // Ghost positions inside the house
        ghosts: [
            {x: doorPos.x-1, y: doorPos.y+1}, // Blinky (33)
            {x: doorPos.x, y: doorPos.y+1},   // Pinky (34)
            {x: doorPos.x+1, y: doorPos.y+1}, // Inky (35)
            {x: doorPos.x, y: doorPos.y+2}    // Sue (36)
        ],
        // Pacman starts on a valid position
        pacman: pacmanPos
    };
};

// Add this function to get the entire maze in your numeric format
Map.prototype.getNumericMaze = function() {
    const maze = [];
    for (let y = 0; y < this.numRows; y++) {
        const row = [];
        for (let x = 0; x < this.numCols; x++) {
            row.push(this.getNumericTile(x, y));
        }
        maze.push(row);
    }
    return maze;
