...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        if is_last_batch:
            ...
            # Retrieve all batches for the episode from the message queue
            while True:
                method_frame, header_frame, body = ch.basic_get(queue='HF_upload_queue', auto_ack=True)
                if body is None:
                    break
                batch = pickle.loads(body)
                if batch['episode'] == episode:
                    combined_data['frame'] += batch['frame']
                    combined_data['frames'].extend(batch['frames'])
                    combined_data['actions'].extend(batch['actions'])
                    combined_data['next_frames'].extend(batch['next_frames'])
